<template>
  <div class="rosefinch-message-list">
    <div class="empty-wrap" v-if="!list.length && !list1.length">
    ...
    >
       <SystemItem
        :class="{ last: index === list1.length - 1 }"
...
export default {
  ...
  computed: {
    dataText() {
      return this.list.length > 0 || this.list1.length>0
...
console.log(this.refreshing)
    ...
    async reqMessageList() {
      ...
      try {
        ...
        const   data1 = await messageList(params1);

if (data1.success && data1.data) {
          this.list1.push(...data1.data.data);
          this.page = data1.data.page;
        }
        if (success && data) {
